...
export class ToDoEditDialogComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    // Initialize entry if not provided
    // Check if entry has meaningful data (title or description) to determine if it's existing or new
    const hasEntryData =
      this.entry &&
      (this.entry.title?.trim() || this.entry.description?.trim());

    if (!this.entry || !hasEntryData) {
      // No entry provided or entry is completely empty - this is a new entry
      this.isNew = true;
      const now = Date.now();
      ...
      // Entry was provided with data, so it's an existing entry to edit
      this.isNew = false;
      // Ensure toDoID is set for existing entries (generate if missing)
      if (!this.entry.toDoID || this.entry.toDoID.trim() === '') {
        this.entry.toDoID = ''; // Keep as empty, will be generated on save
      }
      console.log(
        '🎯 Loading entry for edit:',
        this.entry.toDoID,
        this.entry.title
      );
    }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // When entry input changes, update isNew flag and specific target types
    if (changes['entry']) {
      const hasEntryData =
        this.entry &&
        (this.entry.title?.trim() || this.entry.description?.trim());

      if (!changes['entry'].firstChange) {
        // Entry changed after initial load
        this.isNew = !hasEntryData;
        console.log(
          '🎯 Entry changed:',
          this.isNew ? 'new entry' : this.entry?.title || 'existing entry'
        );
        this.updateSpecificTargetTypes();
      } else if (this.entry) {
        // Initial load with entry provided
        this.isNew = !hasEntryData;
        console.log(
          '🎯 Initial entry load:',
          this.isNew ? 'new entry' : this.entry?.title || 'existing entry'
