...
import { DialogEditLessonComponent } from '../../dialogs/dialog-edit-lesson/dialog-edit-lesson.component';
import { DialogCreateLessonComponent } from '../../dialogs/dialog-create-lesson/dialog-create-lesson.component';
...
import { TreeNode, D3TreeNode } from '../../../interfaces';
...
@Component({
  selector: 'app-d3-example-pan-to-point',
  standalone: true,
  imports: [
    CommonModule,
  ...
  templateUrl: './d3-example-pan-to-point.html',
  styleUrl: './d3-example-pan-to-point.scss',
})
export class D3ExamplePanToPoint implements OnInit, AfterViewInit, OnDestroy {
  ...
  public toolbarPositions: {
    drawingTools: { x: number; y: number };
    ...
    nodesList: { x: number; y: number };
  } = {
    drawingTools: { x: 20, y: 20 },
    ...
    nodesList: { x: window.innerWidth - 200, y: 300 },
  };
  ...
  private constrainToolbarsToWindow() {
    ...
    const toolbarSizes: Record<string, { width: number; height: number }> = {
      drawingTools: { width: 320, height: 300 },
      ...
      nodesList: { width: 180, height: 350 },
    };
  ...
  private selectNodesInLasso() {
    ...
    // Get all tree nodes
    const nodes = this.svg.selectAll('.tree-node');
    console.log('Found tree nodes:', nodes.size());
    ...
    nodes.each((d: any, i: number, nodeArray: any) => {
      ...
      console.log(`Checking tree node ${nodeId} at position (${cx}, ${cy})`);
  ...
  // Virtual scrolling method
  onVirtualScroll(event: Event): void {
    const target = event.target as HTMLElement;
    this.virtualScrollTop = target.scrollTop;
  }

  // Get the actual index in the full list from virtual list index
  getActualNodeIndex(virtualIndex: number): number {
    return this.visibleStartIndex + virtualIndex;
  }

  updateNodeSelectionVisuals() {
    if (!this.svg) return;

    // Update all tree node circles
    this.svg
      .selectAll('.tree-node')
      .style('stroke', (d: any) =>
        this.isNodeSelected(d.id)
          ? '#ff6b6b'
          : d.depth === 0
          ? '#d84315'
          : '#1565c0'
      ...
      .style('stroke-width', (d: any) =>
        this.isNodeSelected(d.id) ? '4px' : '2px'
      )
      .style('fill-opacity', (d: any) =>
        this.isNodeSelected(d.id) ? 1.0 : 0.8
      );
    ...
    this.svg
      ...
      .style('font-weight', (d: any) =>
        this.isNodeSelected(d.id) ? 'bold' : d.depth === 0 ? 'bold' : 'normal'
      )
      .style('fill', (d: any) =>
        this.isNodeSelected(d.id) ? '#ff6b6b' : '#fff'
      );
  }
  ...
  public saveNewLesson(): void {
    ...
    const newLesson: ILesson = {
      LessonName: this.newLessonName.trim(),
  ...
  public saveEditedLesson(): void {
    if (!this.selectedLesson || !this.editLessonName.trim()) {
      return;
    }

    const currentName = this.selectedLesson.LessonName;
    const newName = this.editLessonName.trim();
    ...
    console.log(`Renamed lesson from "${currentName}" to "${newName}"`);
    this.closeEditLessonDialog();
  }
  ...
  private redrawNodes() {
    ...
    // Update tree using enter-update-exit pattern
    this.updateTree(this.treeNodes, this.treeLinks);
  }
  ...
  private updateTree(nodes: D3TreeNode[], links: any[]) {
    ...
    this.g.selectAll('.tree-link').remove();
    this.g.selectAll('.tree-node').remove();
    this.g.selectAll('.tree-label').remove();
  ...
  private drawTreeNodes(nodes: D3TreeNode[]) {
    this.g
      ...
      .attr('cy', (d: any) => d.y)
      .attr('r', (d: any) => d.r)
      .attr('fill', (d: any) => (d.depth === 0 ? '#ff6b35' : '#2196f3')) // Root node in different color
      ...
      .on('click', (event: any, d: any) => {
        if (this.drawingMode === 'pan') {
          console.log('tree node click', d.id, event);
          this.selectedNode = d.id;
          event.stopPropagation();
        } else if (this.drawingMode === 'select') {
          console.log('tree node select click', d.id, event);
  ...
  private drawTreeLabels(nodes: D3TreeNode[]) {
    this.g
      ...
      .on('click', (event: any, d: any) => {
        if (this.drawingMode === 'pan') {
          console.log('tree label click', d.id, event);
          this.selectedNode = d.id;
          event.stopPropagation();
        } else if (this.drawingMode === 'select') {
          console.log('tree label select click', d.id, event);
          this.toggleNodeSelection(d.id);
          event.stopPropagation();
        }
      });
