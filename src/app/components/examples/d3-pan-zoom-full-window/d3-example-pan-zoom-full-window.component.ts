...
@Component({
  selector: 'app-d3-pan-zoom-full-window',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './d3-example-pan-zoom-full-window.component.html',
  styleUrls: ['./d3-example-pan-zoom-full-window.component.scss'],
})
export class D3ExamplePanZoomFullWindowComponent implements AfterViewInit {
  ...
  private drawDegreeCircle() {
    ...
    const availableWidth = this.width;
    const availableHeight = this.height - 120; // Account for control panels (top/bottom)
    const contentRadius = Math.min(availableWidth, availableHeight) * 0.35; // Content boundary
