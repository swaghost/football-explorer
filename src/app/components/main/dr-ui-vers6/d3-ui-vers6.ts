...
@Component({
  ...
  templateUrl: './d3-ui-vers6.html',
  styleUrls: ['./d3-ui-vers6-specific.scss'],
})
export class D3UIV6
  implements OnInit, OnDestroy, AfterViewInit, AfterViewChecked
{
  ...
  // Track if lesson toolbars were manually closed by user to prevent auto-opening
  private lessonRunnerV2ManuallyClosed = false;
  private lessonBuilderV2ManuallyClosed = false;
  ...
  private startDrawing(event: MouseEvent) {
    ...
    if (this.drawingMode === 'pencil') {
      ...
      this.currentShape = {
        ...
        fontFamily: this.textFontFamily || 'Arial',
  ...
  panToNode(node: D3TreeNode): void {
    if (node && node.x !== undefined && node.y !== undefined) {
      this.panToNodeService.panToNode(
        {
          nodeX: node.x,
          nodeY: node.y,
        ...
        this.panY,
        {
          onUpdate: (panX: number, panY: number) => {
            this.panX = panX;
            this.panY = panY;
            this.applyTransform();
          },
          onComplete: () => {
            // Animation complete - could add additional logic here if needed
          },
        }
      );
    }
  ...
  private updateTreeVisibility() {
    ...
    // Toggle visibility of tree links with smooth transition (use 0.6 opacity when visible)
    this.svg
      .selectAll('.tree-link')
      .transition()
      .duration(2000) // 2 second transition
      .ease(d3.easeCubicInOut) // Smooth easing
      .style('opacity', this.treeVisible ? 0.6 : 0);
  ...
  private generateTreeLayout(treeData: TreeNode): {
    ...
    const nodes = treeWithLayout.descendants().map((d: any) => {
  ...
  // Initialize all visualization layout functions (imported from D3ExampleTransitions)
  ...
  private updateVisualization(): void {
    ...
    switch (this.selectedVisualization) {
      case 'radialTree':
        layout = this.radialTreeLayout(hierarchy);
        nodes = layout.descendants().map((d: any) => ({
          ...d,
          id: d.data.id,
          videoUrl: d.data.videoUrl, // Copy videoUrl to node level for easier template access
      ...
      case 'radialCluster':
        layout = this.radialClusterLayout(hierarchy);
        nodes = layout.descendants().map((d: any) => ({
          ...d,
          id: d.data.id,
          videoUrl: d.data.videoUrl, // Copy videoUrl to node level for easier template access
      ...
      case 'treeHorizontal':
        layout = this.horizontalTreeLayout(hierarchy);
        nodes = layout.descendants().map((d: any) => ({
          ...d,
          id: d.data.id,
          videoUrl: d.data.videoUrl, // Copy videoUrl to node level for easier template access
        ...
        links = layout.links().map((d: any) => ({
          source: { x: margin.left + d.source.y, y: margin.top + d.source.x },
          target: { x: margin.left + d.target.y, y: margin.top + d.target.x },
        }));
        break;

      case 'treeVertical':
        layout = this.verticalTreeLayout(hierarchy);
        nodes = layout.descendants().map((d: any) => ({
          ...d,
          id: d.data.id,
          videoUrl: d.data.videoUrl, // Copy videoUrl to node level for easier template access
        ...
        links = layout.links().map((d: any) => ({
          source: { x: margin.left + d.source.x, y: margin.top + d.source.y },
          target: { x: margin.left + d.target.x, y: margin.top + d.target.y },
        }));
        break;

      case 'clusterHorizontal':
        layout = this.horizontalClusterLayout(hierarchy);
        nodes = layout.descendants().map((d: any) => ({
          ...d,
          id: d.data.id,
          videoUrl: d.data.videoUrl, // Copy videoUrl to node level for easier template access
      ...
      case 'clusterVertical':
        layout = this.verticalClusterLayout(hierarchy);
        nodes = layout.descendants().map((d: any) => ({
          ...d,
          id: d.data.id,
          videoUrl: d.data.videoUrl, // Copy videoUrl to node level for easier template access
      ...
      default:
        console.warn('Unknown visualization type:', this.selectedVisualization);
        return;
    }

    // Update tree nodes and links
    this.treeNodes = nodes;
    this.treeLinks = links;

    // Trigger change detection to avoid ExpressionChangedAfterItHasBeenCheckedError
    this.cdr.detectChanges();

    // Apply smooth transitions using enter/update/exit pattern
    this.transitionToNewLayout(nodes, links);
  }
  ...
  private transitionToNewLayout(nodes: any[], links: any[]): void {
    ...
    linkEnter
      .merge(linkSelection)
      .transition(t)
      .style('opacity', this.treeVisible ? 0.6 : 0) // Use 0.6 for links like static method
      .style('stroke', this.isDarkMode ? '#555' : '#ccc')
      .attr('d', (d: any) => {
    ...
    nodeEnter
      ...
      .attr('r', (d: any) => d.r)
      .style('fill', (d: any) => this.getNodeColor(d.id))
      .style('stroke', this.isDarkMode ? '#fff' : '#333')
    ...
    nodeUpdate
      .transition(t)
      .style('opacity', this.treeVisible ? 1 : 0) // Respect tree visibility
  ...
  private getNodeColor(nodeId: string): string {
    ...
    return this.colorsService.getDefaultNodeStyle(this.isDarkMode).nodeColor; // Default node color
  }
  ...
  private drawTreeLinks(links: any[]) {
    ...
    linkEnter
      ...
      .attr('stroke', this.isDarkMode ? '#666' : '#999')
      .attr('stroke-width', 2)
      .style('opacity', this.treeVisible ? 0.6 : 0); // Respect tree visibility with smooth transition
  }
    ...
    this.treeNodesGroup
      ...
      .attr('fill', (d: any) =>
        d.depth === 0
          ...
          : this.colorsService.getDefaultNodeStyle(this.isDarkMode).nodeColor
  ...
  public hideToolbar(toolbarType: string): void {
    console.log(`🎯 [hideToolbar] Called for: ${toolbarType}`);
    ...
    // Track if lesson toolbars were manually closed
    if (toolbarType === 'lessonRunnerV2') {
      this.lessonRunnerV2ManuallyClosed = true;
      console.log('🏃 Lesson Runner V2 manually closed - will not auto-open');
    }

    if (toolbarType === 'lessonBuilderV2') {
      this.lessonBuilderV2ManuallyClosed = true;
      console.log('📚 Lesson Builder V2 manually closed - will not auto-open');
    }
  }
  ...
  public onToolbarVisibilityChange(
    ...
    console.log(`🎯 [onToolbarVisibilityChange] ${toolbarType} = ${visible}`);
  ...
  public showToolbar(toolbarType: string): void {
    ...
    // Reset the manually-closed flags when user explicitly opens the toolbars
    if (toolbarType === 'lessonRunnerV2') {
      this.lessonRunnerV2ManuallyClosed = false;
      console.log('🏃 Lesson Runner V2 opened - auto-open allowed again');
    }

    if (toolbarType === 'lessonBuilderV2') {
      this.lessonBuilderV2ManuallyClosed = false;
      console.log('📚 Lesson Builder V2 opened - auto-open allowed again');
    }
  }
  ...
  public showAllToolbars(): void {
    this.store.dispatch(new SetAllToolbarVisibility(true));
  }
  ...
  public takeScreenshot(options: {
    ...
    // Get the full SVG viewBox or bounding box
    ...
    // Create a canvas element with the full SVG dimensions
  ...
  public onLessonBuilderV2ToggleExpanded(): void {
    // Toggle and persist Lesson Builder V2 expand state
    const currentState = this.store.selectSnapshot(
      SketchState.getToolbarExpandStates
    );
    const newExpandState = !currentState.lessonBuilderV2;
    console.log('📌 Lesson Builder V2 toggle expanded:', newExpandState);
    this.store.dispatch(
      new SketchActions.SetToolbarExpandState('lessonBuilderV2', newExpandState)
    );
  }

  public onLessonRunnerV2ToggleExpanded(): void {
    // Toggle and persist Lesson Runner V2 expand state
    const currentState = this.store.selectSnapshot(
      SketchState.getToolbarExpandStates
    );
    const newExpandState = !currentState.lessonRunnerV2;
    console.log('📌 Lesson Runner V2 toggle expanded:', newExpandState);
    this.store.dispatch(
      new SketchActions.SetToolbarExpandState('lessonRunnerV2', newExpandState)
    );
  }
